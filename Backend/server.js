        ...
        if (data.length > 0) {
            ...
            bcrypt.compare(password.toString(), hashedPassword, (err, response) => { //compare hashed database password to entered password hashed
                ...
                if (response) {
                    ...
                    if (role === 'admin') {
                        return res.json({ message: "Login successful...", redirectUrl: "/event-management", userId });
                    } else {
                        return res.json({ message: "Login successful...", redirectUrl: `/user/${userId}`, userId });
                    }
                } else {
...
app.put('/events/update/:event_id', (req, res) => {
    ...
    db.query(sql, [...values, event_id], (err, data) => {
        ...
        db.query(notifyQuery, [req.body.name, event_id], (notifyErr) => {
            if (notifyErr) return res.status(500).json({ error: notifyErr.message });
...
app.put('/volunteers/match/:user_id', (req, res) => {
    ...
    db.query(sql, [...values, user_id], (err, data) => {
        ...
        db.query(notifyQuery, [req.body.matchedEvent, req.body.matchedEvent], (notifyErr) => {
            if (notifyErr) return res.status(500).json({ error: notifyErr.message });
...
app.get('/notifications', (req, res) => {
    ...
    db.query(query, [userId], (error, results) => {
        if (error) return res.status(500).json({ message: "Error retrieving notifications" });
        res.json(results);
    });
});
